...
export const LocationSchema = SensorDataSchema.extend({
  id: z.string().openapi({ example: 'sensor-1', description: '感測器設備 ID' }),
  name: z.string().openapi({ example: '摩茲工寮', description: '感測器顯示名稱' }),
}).openapi('Location');
...
api.openapi(getLocationsRoute, async (c) => {
  try {
...
api.openapi(getLocationByIdRoute, async (c) => {
  try {
    ...
    console.error(`[Error] GET /locations/${c.req.param('id')}:`, error);
    return c.text('無法抓取裝置資訊，請稍後再試。', 500);
  }
});

// GET /locations/:id/history
const getLocationHistoryRoute = createRoute({
  method: 'get',
  path: '/{id}/history',
  summary: '取得特定位置的歷史數據',
...
api.openapi(getLocationHistoryRoute, async (c) => {
  try {
    const id = c.req.valid('param').id;
    ...
    console.error(`[Error] GET /locations/${c.req.param('id')}/history:`, error);
