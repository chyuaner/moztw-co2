...
export const SpaceApiValueSchema = z.object({
  ...
  location: z.string().openapi({ example: 'sensor-1' }),
  name: z.string().openapi({ example: '摩茲工寮' }),
...
// GET /sensors
const getSensorsRoute = createRoute({
  method: 'get',
  path: '/',
...
api.openapi(getSensorsRoute, async (c) => {
  try {
...
// GET /sensors/:id
const getSensorByIdRoute = createRoute({
  method: 'get',
  path: '/{id}',
...
api.openapi(getSensorByIdRoute, async (c) => {
  try {
