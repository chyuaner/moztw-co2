...
const RoomCard: FC<{ location: any, index: number }> = ({ location, index }) => {
  ...
  return (
    <div id={`room-card-${location.id}`} className="card lg:card-side bg-white shadow-sm mb-5 overflow-hidden h-auto lg:h-[280px] border border-gray-200 rounded-xl">
...
export const Base: FC<PropsWithChildren<{ title?: string, baseData?: BaseData }>> = (props) => {
  ...
  return (
    ...
    <body className="bg-gray-50 text-gray-900 m-0 font-sans min-h-screen">
  ...
  return (
    <Base title="三個房間 環境監測">
      <div className="max-w-[1200px] mx-auto">
        <div className="flex flex-col md:flex-row justify-between md:items-end mb-6 pb-3 border-b-2 border-gray-200 gap-4 md:gap-0">
          ...
          <div className="flex gap-2 bg-gray-100 p-1 rounded-lg w-full md:w-auto">
            {Object.entries(timeLabels).map(([key, label]) => (
              <button 
                key={key} 
                data-timeframe={key}
                className={`timeframe-btn flex-1 md:flex-none px-3 py-1.5 rounded-md text-sm transition-all duration-200 ${key === '6h' ? 'bg-white text-gray-900 shadow font-semibold' : 'text-gray-500 hover:text-gray-900'}`}
              >{label}</button>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center gap-4 mt-4 text-xs text-gray-500 border-t border-gray-200 pt-3">
...
export const ApiDocPage: FC = () => {
  return (
    <Base title="API Documentation">
      ...
      <div className="max-w-[1200px] mx-auto bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden p-2">
